...
/*****************Modified by ENSG / PPMD / FRANCE Student***********************************/
    ...
    if (fichier != NULL)
    {
        fputs("La version de RTKLIB est : ", fichier);
        /*fputs(pos[0],fichier);*/
        fputs(VER_RTKLIB,fichier);
        fputs("\n", fichier);
        /*fprintf(fichier,"%lf\n",VER_RTKLIB);*/
        /*position XYZ*/
        fprintf(fichier,"%lf\n",rtk.sol.rr[0]);
        ...
        fprintf(fichier,"%lf\n",b);
        fprintf(fichier,"%lf\n",c);
        fprintf(fichier,"%02.0f:%02.0f:%04.1f\n",rt[0],rt[1],rt[2]);
        fprintf(fichier,"%s\n",mode[rtk.opt.mode]);
        fprintf(fichier,"%s\n",sol[rtkstat]);
        fprintf(fichier,"%d\n",nsat0);
        fprintf(fichier,"%d\n",nsat1);
        fprintf(fichier,"%d\n",rtk.sol.ns);
        fprintf(fichier,rtk.sol.time.time?tstr:"-");
        fprintf(fichier,"\n");
...
/*---------------------Won't be used even in "ouvreVT"-----------------------------------*/
    while (!intflg) {
        
        /* open console */
        fprintf(stderr,"on est dans rtkrcv.c juste avant test if !vt_open...-------    ");
        /*if (!vt_open(&vt,port,dev)) return 0;*/
        /*fprintf(stderr,"on est juste après");*/
...
/*---------------------------------------------------------------------------------------------*/
    ...
    fprintf(stderr,"on est passé ici");
